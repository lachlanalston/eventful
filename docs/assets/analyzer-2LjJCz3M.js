...
`)}function ee(e){const t={Critical:0,Error:0,Warning:0,Info:0,Verbose:0};for(const n of e)t[n.severity]=(t[n.severity]||0)+1;return{total:e.length,...t}}function He(){return{total:0,Critical:0,Error:0,Warning:0,Info:0,Verbose:0}}function _e(e,t){let n=100;const a=ee(e);n-=Math.min(a.Critical*15,40),n-=Math.min(a.Error*3,25),n-=Math.min(a.Warning*.5,10),n-=t.length*12;for(const s of t)s.report.confidence==="high"?n-=8:s.report.confidence==="medium"&&(n-=4);return Math.max(0,Math.min(100,Math.round(n)))}function Ve(e){const t=new Set;return e.filter(n=>{const a=`${n.anchor.id}-${Math.floor(n.anchor.timestamp/1e3)}`;return t.has(a)?!1:(t.add(a),!0)})}function te(e){return e.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}$e();document.querySelectorAll(".theme-btn").forEach(e=>e.addEventListener("click",ke));const A=document.getElementById("upload-section"),ne=document.getElementById("processing-section"),se=document.getElementById("results-section"),g=document.getElementById("drop-zone"),I=document.getElementById("file-input"),G=document.getElementById("processing-text"),F=document.getElementById("overview-grid"),S=document.getElementById("incidents-section"),k=document.getElementById("event-table-wrap"),R=document.getElementById("event-log-filters-wrap"),N=document.getElementById("new-analysis-btn"),z=document.getElementById("results-sub");let C=[],x=new Map;I==null||I.addEventListener("change",e=>{var n;const t=(n=e.target.files)==null?void 0:n[0];t&&ie(t)});g==null||g.addEventListener("dragover",e=>{e.preventDefault(),g.classList.add("drag-over")});g==null||g.addEventListener("dragleave",()=>g.classList.remove("drag-over"));g==null||g.addEventListener("drop",e=>{var n;e.preventDefault(),g.classList.remove("drag-over");const t=(n=e.dataTransfer.files)==null?void 0:n[0];t&&ie(t)});N==null||N.addEventListener("click",Ke);var Q;(Q=document.getElementById("lp-backdrop"))==null||Q.addEventListener("click",M);var J;(J=document.getElementById("lp-close"))==null||J.addEventListener("click",M);document.addEventListener("keydown",e=>{e.key==="Escape"&&M()});async function ie(e){if(!e.name.toLowerCase().endsWith(".xml")&&e.type!=="text/xml"&&e.type!=="application/xml"){Y("Please upload an XML file exported from Windows Event Viewer.");return}T(`Reading ${e.name}…`);try{const t=await e.text();T("Parsing events…"),await P();const n=be(t);T(`Analysing ${n.length.toLocaleString()} events…`),await P();const a=Ne(n);C=n,x=new Map;for(const s of n)x.set(s.id,(x.get(s.id)||0)+1);T("Building report…"),await P(),Ge(a,e.name)}catch(t){Y(t.message||"Failed to parse file."),D(A)}}function P(){return new Promise(e=>setTimeout(e,16))}function D(e){[A,ne,se].forEach(t=>{t&&(t.hidden=!0)}),e&&(e.hidden=!1)}function T(e){G&&(G.textContent=e),D(ne)}function Ke(){C=[],x=new Map,I&&(I.value=""),D(A)}function Ge(e,t){const{incidents:n,healthScore:a,computerName:s,stats:i}=e,o=document.querySelector(".results-title");if(o&&(o.textContent=t.replace(/\.xml$/i,"")),z){const c=[];s&&c.push(s),c.push(`${i.total.toLocaleString()} events`),n.length&&c.push(`${n.length} incident${n.length!==1?"s":""} detected`),z.textContent=c.join(" · ")}Fe(a,i),ze(n),Ze(C);const r=document.getElementById("tab-inc-count"),d=document.getElementById("tab-evt-count");r&&(r.textContent=n.length),d&&(d.textContent=i.total.toLocaleString()),document.querySelectorAll(".analyzer-tab").forEach(c=>{c.addEventListener("click",()=>{document.querySelectorAll(".analyzer-tab").forEach(v=>v.classList.remove("active")),c.classList.add("active");const u=c.dataset.tab;document.getElementById("incidents-section").hidden=u!=="incidents",document.getElementById("events-panel").hidden=u!=="events"})}),D(se)}function Fe(e,t){if(!F)return;const n=e>=80?"#34d399":e>=60?"#f59e0b":"#f43f5e",a=e>=80?"Good":e>=60?"Degraded":"Critical";F.innerHTML=`
  ...
  `}function ze(e){if(S){if(!e.length){S.innerHTML=`
    ...
    `;return}S.innerHTML=e.map((t,n)=>Xe(t)).join(""),S.querySelectorAll(".incident-toggle").forEach(t=>{t.addEventListener("click",n=>{if(n.target.closest("[data-lookup-id]"))return;const s=t.closest(".incident-card").querySelector(".incident-body"),i=t.querySelector(".incident-chevron"),o=!s.hidden;s.hidden=o,i.classList.toggle("open",!o)})}),S.querySelectorAll(".copy-summary-btn").forEach(t=>{t.addEventListener("click",()=>{const n=t.dataset.summary;navigator.clipboard.writeText(n).then(()=>{t.textContent="Copied!",t.classList.add("copied"),setTimeout(()=>{t.textContent="Copy for ticket",t.classList.remove("copied")},2e3)})})}),S.querySelectorAll(".evidence-item").forEach(t=>{t.addEventListener("click",n=>{if(n.stopPropagation(),n.target.closest("[data-lookup-id]"))return;const s=t.closest(".evidence-wrap").querySelector(".evidence-detail"),i=t.querySelector(".ev-expand-chevron"),o=!s.hidden;s.hidden=o,t.classList.toggle("expanded",!o),i&&(i.textContent=o?"▶":"▼")})}),S.querySelectorAll(".timeline-item").forEach(t=>{t.addEventListener("click",n=>{if(n.stopPropagation(),n.target.closest("[data-lookup-id]"))return;const a=t.closest(".timeline-item-wrap");if(!a)return;const s=a.querySelector(".timeline-detail");if(!s)return;const i=t.querySelector(".tl-expand-chevron"),o=!s.hidden;s.hidden=o,i&&(i.textContent=o?"▶":"▼")})}),S.querySelectorAll(".ev-advanced-toggle").forEach(t=>{t.addEventListener("click",n=>{n.stopPropagation();const s=t.closest(".ev-inline-detail").querySelector(".ev-advanced-section").classList.toggle("ev-advanced-open");t.textContent=s?"Advanced ▲":"Advanced ▼"})}),S.querySelectorAll(".ev-copy-btn").forEach(t=>{t.addEventListener("click",n=>{var a;n.stopPropagation(),(a=navigator.clipboard)==null||a.writeText(t.dataset.copy).then(()=>{t.classList.add("copied"),setTimeout(()=>t.classList.remove("copied"),2e3)})})}),S.querySelectorAll("[data-lookup-id]").forEach(t=>{t.addEventListener("click",n=>{n.stopPropagation(),ae(t.dataset.lookupId)})})}}function Xe(e,t){var v,h;const{anchor:n,windowEvents:a,topContributors:s,signatureResult:i,report:o}=e,r=(v=i[0])==null?void 0:v.signature,d=o.confidence,c=ct(n.severity),u=d==="high"?"conf-high":d==="medium"?"conf-medium":"conf-low";return`
      ...
      <div class="incident-body" hidden>
        ...
        <div class="incident-section">
          ...
          <div class="evidence-list">
            ${s.slice(0,6).map(({event:m,score:E})=>`
              <div class="evidence-wrap">
                <div class="evidence-item">
                  ...
                  <span class="ev-provider">${p(L(m.provider))}</span>
                  <span class="ev-time">${ue(m.timestamp)}</span>
                  <span class="ev-score" title="Relevance score">${E}</span>
                  <span class="ev-expand-chevron">▶</span>
                </div>
                <div class="evidence-detail" hidden>${re(m)}</div>
              </div>
            `).join("")}
          </div>
        </div>
        ...
        <div class="incident-section">
          ...
          <ol class="next-steps-list">
            ${o.nextSteps.map(m=>`<li>${p(m)}</li>`).join("")}
          </ol>
        </div>
        `:""}
            ...
            <div class="timeline-item-wrap">
              <div class="timeline-item ${o?"timeline-anchor":""}">
                ...
                <div class="tl-content">
                  <span class="tl-time">${ue(i.timestamp)}</span>
                  <span class="tl-id" data-lookup-id="${i.id}" title="Look up Event ${i.id}">${i.id}</span>
                  <span class="tl-provider">${p(L(i.provider))}</span>
                  ${o?'<span class="tl-anchor-label">ANCHOR</span>':""}
                </div>
                <span class="tl-expand-chevron">▶</span>
              </div>
              <div class="timeline-detail" hidden>${re(i)}</div>
            </div>
          `}).join("")}
      </div>
    </div>
  `}const Qe=15,Je=new Set(["Microsoft-Windows-TaskScheduler","Microsoft-Windows-WindowsUpdateClient","Microsoft-Windows-Bits-Client","Microsoft-Windows-GroupPolicy","Microsoft-Windows-UserPnp","Microsoft-Windows-WER-SystemErrorReporting","Microsoft-Windows-Diagnostics-Performance","Microsoft-Windows-DistributedCOM","Microsoft-Windows-Security-SPP","Microsoft-Windows-Defrag","Microsoft-Windows-Power-Troubleshooter"]),X={Critical:0,Error:1,Warning:2,Info:3,Verbose:4},l={sortCol:"timestamp",sortDir:"asc",page:0,pageSize:100,query:"",severities:new Set,provider:"",channel:"",fromTime:"",toTime:"",hideNoisy:!1,expandedIds:new Set};function Ze(e){var r,d;if(!R||!k)return;Object.assign(l,{sortCol:"timestamp",sortDir:"asc",page:0,query:"",severities:new Set,provider:"",channel:"",fromTime:"",toTime:"",hideNoisy:!1,expandedIds:new Set});const t=[...new Set(e.map(c=>c.provider).filter(Boolean))].sort(),n=[...new Set(e.map(c=>c.channel).filter(Boolean))].sort(),a=c=>c?new Date(c-c.getTimezoneOffset()*6e4).toISOString().slice(0,16):"",s=(r=e[0])==null?void 0:r.timestamp,i=(d=e[e.length-1])==null?void 0:d.timestamp;R.innerHTML=`
    <div class="event-log-filters">
      ...
      <select id="tbl-provider" class="filter-control filter-control-select">
        ...
        ${t.map(c=>`<option value="${p(c)}">${p(L(c))}</option>`).join("")}
      </select>
  ...
  `;const o=(c,u,v)=>{var h;return(h=document.getElementById(c))==null?void 0:h.addEventListener(u,v)};o("tbl-query","input",c=>{l.query=c.target.value,l.page=0,w()}),R.querySelectorAll(".tbl-sev-cb").forEach(c=>{c.addEventListener("change",()=>{c.checked?l.severities.add(c.value):l.severities.delete(c.value),c.closest(".sev-chip").classList.toggle("active",c.checked),l.page=0,w()})}),o("tbl-provider","change",c=>{l.provider=c.target.value,l.page=0,w()}),o("tbl-channel","change",c=>{l.channel=c.target.value,l.page=0,w()}),o("tbl-from","change",c=>{l.fromTime=c.target.value,l.page=0,w()}),o("tbl-to","change",c=>{l.toTime=c.target.value,l.page=0,w()}),o("tbl-noise","click",c=>{l.hideNoisy=!l.hideNoisy,l.page=0,c.target.classList.toggle("active",l.hideNoisy),c.target.textContent=l.hideNoisy?"Show noise":"Hide noise",w()}),o("tbl-csv","click",()=>tt(oe())),w()}function oe(){const e=l.query.toLowerCase(),t=l.fromTime?new Date(l.fromTime).getTime():null,n=l.toTime?new Date(l.toTime).getTime():null;let a=C.filter(s=>{if(l.severities.size>0&&!l.severities.has(s.severity)||l.provider&&s.provider!==l.provider||l.channel&&s.channel!==l.channel||t!==null&&s.timestamp<t||n!==null&&s.timestamp>n||l.hideNoisy&&Je.has(s.provider))return!1;if(e){const i=/^\d+$/.test(e)?parseInt(e,10):null;if(i!==null){if(s.id!==i)return!1}else if(!`${s.id} ${s.provider} ${s.channel} ${s.message} ${s.severity}`.toLowerCase().includes(e))return!1}return!0});return a.sort((s,i)=>{let o=0;switch(l.sortCol){case"timestamp":o=s.timestamp-i.timestamp;break;case"severity":o=(X[s.severity]??9)-(X[i.severity]??9);break;case"id":o=s.id-i.id;break;case"provider":o=(s.provider||"").localeCompare(i.provider||"");break}return l.sortDir==="asc"?o:-o}),a}function w(){if(!k)return;const e=oe(),t=e.length,n=Math.max(0,Math.ceil(t/l.pageSize)-1);l.page=Math.min(l.page,n);const a=l.page*l.pageSize,s=e.slice(a,a+l.pageSize);if(!t){k.innerHTML='<div class="table-empty">No events match the current filters.</div>';return}const i=d=>`<span class="sort-arrow ${l.sortCol===d?"active":""}">${l.sortCol===d?l.sortDir==="asc"?"↑":"↓":"↕"}</span>`,o=d=>l.sortCol===d?"sort-active":"";k.innerHTML=`
    ...
    <table class="event-table">
      ...
      <tbody>${s.map(d=>et(d)).join("")}</tbody>
    </table>
  `,k.querySelectorAll("th[data-sort]").forEach(d=>{d.addEventListener("click",()=>{const c=d.dataset.sort;l.sortDir=l.sortCol===c&&l.sortDir==="asc"?"desc":"asc",l.sortCol=c,l.page=0,w()})});const r=(d,c)=>{var u;return(u=document.getElementById(d))==null?void 0:u.addEventListener("click",c)};r("pg-first",()=>{l.page=0,w()}),r("pg-prev",()=>{l.page--,w()}),r("pg-next",()=>{l.page++,w()}),r("pg-last",()=>{l.page=n,w()}),k.querySelectorAll("tbody tr[data-record]").forEach(d=>{d.addEventListener("click",c=>{if(c.target.closest(".table-event-id"))return;const u=parseInt(d.dataset.record,10);l.expandedIds.has(u)?l.expandedIds.delete(u):l.expandedIds.add(u),w()})}),k.querySelectorAll(".table-event-id").forEach(d=>{d.addEventListener("click",c=>{c.stopPropagation(),ae(d.dataset.lookupId)})}),k.querySelectorAll(".ev-advanced-toggle").forEach(d=>{d.addEventListener("click",c=>{c.stopPropagation();const v=d.closest(".ev-detail-inner").querySelector(".ev-advanced-section").classList.toggle("ev-advanced-open");d.textContent=v?"Advanced ▲":"Advanced ▼"})}),k.querySelectorAll(".ev-copy-btn").forEach(d=>{d.addEventListener("click",c=>{var u;c.stopPropagation(),(u=navigator.clipboard)==null||u.writeText(d.dataset.copy).then(()=>{d.classList.add("copied"),setTimeout(()=>d.classList.remove("copied"),2e3)})})})}function et(e){const t=l.expandedIds.has(e.recordId),n=e.severity.toLowerCase(),a=Object.keys(e.data||{}),s=e.message?p(e.message.substring(0,150))+(e.message.length>150?"…":""):'<span style="color:var(--text3);font-style:italic">no message</span>',i=`
    <tr class="ev-row-${n}${t?" row-expanded":""}" data-record="${e.recordId}">
      ...
      <td class="ev-col-provider" title="${p(e.provider)}">${p(L(e.provider))}</td>
      <td class="ev-col-channel">${p(e.channel)}</td>
      <td class="ev-col-message">${s}</td>
    </tr>`;if(!t)return i;const o=e.taskName||e.task||null,r=le(e.opcode,e.opcodeName),d=pe(e.keywords,e.keywordNames),c=ce(e.userSID),u=x.get(e.id)||1,v=[["Time (local)",e.timestamp.toLocaleString()],["Time (UTC)",e.timestamp.toISOString()],["Provider",e.provider],["Channel",e.channel],["Computer",e.computer],["Record ID",e.recordId||null],["User SID",c],["Process ID",e.processId||null],["Thread ID",e.threadId||null],["Activity ID",e.activityId],["Related Act. ID",e.relatedActivityId],["Task",o],["Opcode",r],["Keywords",d]].filter(([,y])=>y),h=[["Raw Level",String(e.levelNum)],["Raw Task",e.task],["Raw Opcode",e.opcode],["Raw Keywords",e.keywords],["Version",e.version],["Qualifiers",e.qualifiers],["Provider Desc.",e.providerDescription]].filter(([,y])=>y),m=de(e.message),E=m?`<div class="ev-detail-message-wrap">
        <div class="ev-detail-message">${p(m)}</div>
        <button class="ev-copy-btn" data-copy="${p(e.message)}" title="Copy message">
       ...
       </div>`,f=e.dataAnon||[];return i+`
    <tr class="ev-detail-row">
      <td colspan="7">
        <div class="ev-detail-inner">
          ${u>1?`<div class="ev-occurrence-bar">Event ${e.id} appears <strong>${u}×</strong> in this log</div>`:""}
          ${E}
          <div class="ev-detail-meta">
            ${v.map(([y,b])=>`
              <div class="ev-detail-field">
                <span class="ev-detail-key">${y}</span>
                <span class="ev-detail-val">${p(String(b))}</span>
              </div>`).join("")}
          </div>
          ${a.length||f.length?`
          <div class="ev-detail-data">
            ...
            ${a.map(y=>`
              <div class="ev-detail-data-row">
                <span class="ev-detail-data-key">${p(y)}</span>
                <span class="ev-detail-data-val">${p(String(e.data[y]))}</span>
              </div>`).join("")}
            ${f.map((y,b)=>`
              <div class="ev-detail-data-row">
                <span class="ev-detail-data-key ev-detail-data-key--anon">[${b}]</span>
                <span class="ev-detail-data-val">${p(String(y))}</span>
              </div>`).join("")}
          </div>`:""}
          <div class="ev-detail-actions">
            ...
            ${h.length?'<button class="ev-advanced-toggle">Advanced ▼</button>':""}
          </div>
          ${h.length?`
...
`),i=URL.createObjectURL(new Blob([s],{type:"text/csv;charset=utf-8;"})),o=Object.assign(document.createElement("a"),{href:i,download:`eventful-${new Date().toISOString().slice(0,10)}.csv`});document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(i)}function ae(e){const t=parseInt(e,10),n=document.getElementById("lookup-panel"),a=document.getElementById("lp-body");if(!n||!a)return;const s=Ee.find(o=>o.id===t),i=C.filter(o=>o.id===t);a.innerHTML=nt(t,s,i),n.hidden=!1,a.querySelectorAll(".lp-copy-ps").forEach(o=>{o.addEventListener("click",()=>{navigator.clipboard.writeText(o.dataset.code).then(()=>{o.textContent="Copied!",setTimeout(()=>{o.textContent="Copy"},2e3)})})}),a.querySelectorAll(".lp-show-in-log").forEach(o=>{o.addEventListener("click",()=>{const r=o.dataset.filterId;M(),document.querySelectorAll(".analyzer-tab").forEach(u=>u.classList.remove("active"));const d=document.querySelector('.analyzer-tab[data-tab="events"]');d&&d.classList.add("active"),document.getElementById("incidents-section").hidden=!0,document.getElementById("events-panel").hidden=!1,l.query=r,l.page=0;const c=document.getElementById("tbl-query");c&&(c.value=r),w()})})}function M(){const e=document.getElementById("lookup-panel");e&&(e.hidden=!0)}function nt(e,t,n){var s,i,o;const a=n.length?`<button class="lp-show-in-log" data-filter-id="${e}">Show all ${n.length} occurrence${n.length!==1?"s":""} in All Events →</button>`:"";if(t){const r=((s=t.severity)==null?void 0:s.toLowerCase())??"info";return`
    ...
    <div class="lp-section">
      ...
      ${a}
    </div>`}function re(e){const t=e.taskName||e.task||null,n=le(e.opcode,e.opcodeName),a=pe(e.keywords,e.keywordNames),s=ce(e.userSID),i=e.dataAnon||[],o=Object.keys(e.data||{}),r=x.get(e.id)||1,d=de(e.message),c=[["Time (local)",e.timestamp.toLocaleString()],["Time (UTC)",e.timestamp.toISOString()],["Provider",e.provider],["Channel",e.channel],["Computer",e.computer],["Record ID",e.recordId||null],["User SID",s],["Process ID",e.processId||null],["Thread ID",e.threadId||null],["Activity ID",e.activityId],["Task",t],["Opcode",n],["Keywords",a]].filter(([,v])=>v),u=[["Raw Level",String(e.levelNum)],["Raw Task",e.task],["Raw Opcode",e.opcode],["Raw Keywords",e.keywords],["Version",e.version]].filter(([,v])=>v);return`
    <div class="ev-inline-detail">
      ${r>1?`<div class="ev-occurrence-bar">Event ${e.id} fired <strong>${r}×</strong> in this log</div>`:""}
      ${d?`<div class="ev-detail-message-wrap">
             <div class="ev-detail-message">${p(d)}</div>
             <button class="ev-copy-btn" data-copy="${p(e.message)}" title="Copy message">
               <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
             </button>
      ...
      <div class="ev-inline-grid">
        <div class="ev-detail-meta">
          ${c.map(([v,h])=>`
            <div class="ev-detail-field">
              <span class="ev-detail-key">${v}</span>
              <span class="ev-detail-val">${p(String(h))}</span>
            </div>`).join("")}
        </div>
        ${o.length||i.length?`
        <div class="ev-detail-data">
          ...
          ${o.map(v=>`
            <div class="ev-detail-data-row">
              <span class="ev-detail-data-key">${p(v)}</span>
              <span class="ev-detail-data-val">${p(String(e.data[v]))}</span>
            </div>`).join("")}
          ${i.map((v,h)=>`
            <div class="ev-detail-data-row">
              <span class="ev-detail-data-key ev-detail-data-key--anon">[${h}]</span>
              <span class="ev-detail-data-val">${p(String(v))}</span>
            </div>`).join("")}
        </div>`:""}
      </div>
      <div class="ev-detail-actions">
    ...
    </div>`}const st={"S-1-1-0":"Everyone","S-1-5-7":"Anonymous","S-1-5-18":"SYSTEM","S-1-5-19":"LOCAL SERVICE","S-1-5-20":"NETWORK SERVICE","S-1-5-32-544":"Administrators","S-1-5-32-545":"Users","S-1-5-32-546":"Guests"};function ce(e){if(!e)return null;const t=st[e];return t?`${t} (${e})`:e}const it={2:"The system cannot find the file specified",3:"The system cannot find the path specified",5:"Access is denied",32:"The process cannot access the file because it is being used by another process",1053:"The service did not respond to the start or control request in a timely fashion",1055:"The service database is locked",1056:"An instance of the service is already running",1058:"The service cannot be started — it is disabled or has no enabled devices associated with it",1060:"The specified service does not exist as an installed service",1061:"The service cannot accept control messages at this time",1067:"The process terminated unexpectedly",1068:"The dependency service or group failed to start",1069:"The service did not start due to a logon failure",1072:"The specified service has been marked for deletion",1073:"The specified service already exists",1326:"Logon failure: unknown user name or bad password"};function de(e){return e&&e.replace(/%%(\d+)/g,(t,n)=>{const a=it[+n];return a?`${a} (%%${n})`:t})}const ot={0:"Info",1:"Start",2:"Stop",3:"DC Start",4:"DC Stop",5:"Extension",6:"Reply",7:"Resume",8:"Suspend",9:"Send",240:"Disconnect",241:"Connect"};function le(e,t){return t||(e==null||e===""?null:ot[String(e)]||String(e))}const at={"0x8000000000000000":"Audit Failure","0x4000000000000000":"Audit Success","0x8080000000000000":"Classic, Audit Failure","0x4080000000000000":"Classic, Audit Success","0x0080000000000000":"Classic","0x0000000000000000":"None"};function pe(e,t){return t!=null&&t.length?t.join(", "):e?at[e.toLowerCase()]??e:null}function Y(e){const t=g==null?void 0:g.querySelector(".upload-error");t&&t.remove();const n=document.createElement("div");n.className="upload-error",n.textContent=e,g==null||g.appendChild(n),D(A)}function p(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"):""}function L(e){return e?e.replace(/^Microsoft-Windows-/i,"").replace(/^Microsoft-/i,""):"—"}function ue(e){return e.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}function rt(e){return e.toLocaleString([],{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"})}function ct(e){return`sev-header-${(e==null?void 0:e.toLowerCase())??"info"}`}function dt(e){return{41:"Unexpected System Reboot",6008:"Unexpected Shutdown Detected",1001:"System Crash (BSOD)",1e3:"Application Crash",7024:"Critical Service Failure"}[e.id]??`Event ${e.id}`}
