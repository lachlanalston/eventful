...
`)}function te(e){const t={Critical:0,Error:0,Warning:0,Info:0,Verbose:0};for(const n of e)t[n.severity]=(t[n.severity]||0)+1;return{total:e.length,...t}}function He(){return{total:0,Critical:0,Error:0,Warning:0,Info:0,Verbose:0}}function _e(e,t){let n=100;const a=te(e);n-=Math.min(a.Critical*15,40),n-=Math.min(a.Error*3,25),n-=Math.min(a.Warning*.5,10),n-=t.length*12;for(const s of t)s.report.confidence==="high"?n-=8:s.report.confidence==="medium"&&(n-=4);return Math.max(0,Math.min(100,Math.round(n)))}function Ve(e){const t=new Set;return e.filter(n=>{const a=`${n.anchor.id}-${Math.floor(n.anchor.timestamp/1e3)}`;return t.has(a)?!1:(t.add(a),!0)})}function ne(e){return e.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}$e();document.querySelectorAll(".theme-btn").forEach(e=>e.addEventListener("click",be));const L=document.getElementById("upload-section"),se=document.getElementById("processing-section"),ie=document.getElementById("results-section"),y=document.getElementById("drop-zone"),A=document.getElementById("file-input"),z=document.getElementById("processing-text"),F=document.getElementById("overview-grid"),$=document.getElementById("incidents-section"),k=document.getElementById("event-table-wrap"),x=document.getElementById("event-log-filters-wrap"),P=document.getElementById("new-analysis-btn"),X=document.getElementById("results-sub");let C=[],D=new Map;A==null||A.addEventListener("change",e=>{var n;const t=(n=e.target.files)==null?void 0:n[0];t&&oe(t)});y==null||y.addEventListener("dragover",e=>{e.preventDefault(),y.classList.add("drag-over")});y==null||y.addEventListener("dragleave",()=>y.classList.remove("drag-over"));y==null||y.addEventListener("drop",e=>{var n;e.preventDefault(),y.classList.remove("drag-over");const t=(n=e.dataTransfer.files)==null?void 0:n[0];t&&oe(t)});P==null||P.addEventListener("click",Ke);var J;(J=document.getElementById("lp-backdrop"))==null||J.addEventListener("click",R);var Z;(Z=document.getElementById("lp-close"))==null||Z.addEventListener("click",R);document.addEventListener("keydown",e=>{e.key==="Escape"&&R()});async function oe(e){if(!e.name.toLowerCase().endsWith(".xml")&&e.type!=="text/xml"&&e.type!=="application/xml"){Q("Please upload an XML file exported from Windows Event Viewer.");return}T(`Reading ${e.name}…`);try{const t=await e.text();T("Parsing events…"),await O();const n=ke(t);T(`Analysing ${n.length.toLocaleString()} events…`),await O();const a=Ne(n);C=n,D=new Map;for(const s of n)D.set(s.id,(D.get(s.id)||0)+1);T("Building report…"),await O(),Ge(a,e.name)}catch(t){Q(t.message||"Failed to parse file."),M(L)}}function O(){return new Promise(e=>setTimeout(e,16))}function M(e){[L,se,ie].forEach(t=>{t&&(t.hidden=!0)}),e&&(e.hidden=!1)}function T(e){z&&(z.textContent=e),M(se)}function Ke(){C=[],D=new Map,A&&(A.value=""),M(L)}function Ge(e,t){const{incidents:n,healthScore:a,computerName:s,stats:i}=e,o=document.querySelector(".results-title");if(o&&(o.textContent=t.replace(/\.xml$/i,"")),X){const p=[];s&&p.push(s),p.push(`${i.total.toLocaleString()} events`),n.length&&p.push(`${n.length} incident${n.length!==1?"s":""} detected`),X.textContent=p.join(" · ")}ze(a,i),Fe(n),Ze(C);const r=document.getElementById("tab-inc-count"),d=document.getElementById("tab-evt-count");r&&(r.textContent=n.length),d&&(d.textContent=i.total.toLocaleString()),document.querySelectorAll(".analyzer-tab").forEach(p=>{p.addEventListener("click",()=>{document.querySelectorAll(".analyzer-tab").forEach(f=>f.classList.remove("active")),p.classList.add("active");const u=p.dataset.tab;document.getElementById("incidents-section").hidden=u!=="incidents",document.getElementById("events-panel").hidden=u!=="events"})}),M(ie)}function ze(e,t){if(!F)return;const n=e>=80?"#34d399":e>=60?"#f59e0b":"#f43f5e",a=e>=80?"Good":e>=60?"Degraded":"Critical";F.innerHTML=`
  ...
  `}function Fe(e){if($){if(!e.length){$.innerHTML=`
      ...
      <div class="incident-body" hidden>
        ...
        <div class="incident-section">
          ...
          <div class="evidence-list">
            ${s.slice(0,6).map(({event:m,score:l})=>`
              <div class="evidence-wrap">
                <div class="evidence-item">
                  ...
                  <span class="ev-provider">${v(N(m.provider))}</span>
                  <span class="ev-time">${ve(m.timestamp)}</span>
                  <span class="ev-score" title="Relevance score">${l}</span>
        ...
        <div class="incident-section">
          ...
          <ol class="next-steps-list">
            ${o.nextSteps.map(m=>`<li>${v(m)}</li>`).join("")}
          </ol>
        </div>
        `:""}
            ...
            <div class="timeline-item-wrap">
              <div class="timeline-item ${o?"timeline-anchor":""}">
                ...
                <div class="tl-content">
                  ...
                  <span class="tl-provider">${v(N(i.provider))}</span>
  ...
  `}const Qe=15,Je=new Set(["Microsoft-Windows-TaskScheduler","Microsoft-Windows-WindowsUpdateClient","Microsoft-Windows-Bits-Client","Microsoft-Windows-GroupPolicy","Microsoft-Windows-UserPnp","Microsoft-Windows-WER-SystemErrorReporting","Microsoft-Windows-Diagnostics-Performance","Microsoft-Windows-DistributedCOM","Microsoft-Windows-Security-SPP","Microsoft-Windows-Defrag","Microsoft-Windows-Power-Troubleshooter"]),Y={Critical:0,Error:1,Warning:2,Info:3,Verbose:4},c={sortCol:"timestamp",sortDir:"asc",page:0,pageSize:100,query:"",severities:new Set,providers:new Set,channel:"",fromTime:"",toTime:"",hideNoisy:!1,expandedIds:new Set};document.addEventListener("click",e=>{const t=document.getElementById("tbl-provider-panel"),n=document.getElementById("tbl-provider-btn");t&&!t.hidden&&!t.contains(e.target)&&!(n!=null&&n.contains(e.target))&&(t.hidden=!0,n==null||n.classList.remove("open"))});function Ze(e){var h,m;if(!x||!k)return;Object.assign(c,{sortCol:"timestamp",sortDir:"asc",page:0,query:"",severities:new Set,providers:new Set,channel:"",fromTime:"",toTime:"",hideNoisy:!1,expandedIds:new Set});const t=[...new Set(e.map(l=>l.provider).filter(Boolean))].sort(),n=[...new Set(e.map(l=>l.channel).filter(Boolean))].sort(),a=l=>l?new Date(l-l.getTimezoneOffset()*6e4).toISOString().slice(0,16):"",s=(h=e[0])==null?void 0:h.timestamp,i=(m=e[e.length-1])==null?void 0:m.timestamp;x.innerHTML=`
    <div class="event-log-filters">
          ...
          <div class="provider-option-list">
            ${t.map(l=>`
              <label class="provider-option">
                <input type="checkbox" class="provider-cb" value="${v(l)}" />
                <span class="provider-option-name" title="${v(l)}">${v(N(l))}</span>
              </label>`).join("")}
          </div>
      ...
      <select id="tbl-channel" class="filter-control filter-control-select">
        ...
        ${n.map(l=>`<option value="${v(l)}">${v(l)}</option>`).join("")}
      </select>
  ...
  `;const o=(l,g,b)=>{var w;return(w=document.getElementById(l))==null?void 0:w.addEventListener(g,b)};o("tbl-query","input",l=>{c.query=l.target.value,c.page=0,S()}),x.querySelectorAll(".tbl-sev-cb").forEach(l=>{l.addEventListener("change",()=>{l.checked?c.severities.add(l.value):c.severities.delete(l.value),l.closest(".sev-chip").classList.toggle("active",l.checked),c.page=0,S()})});const r=document.getElementById("tbl-provider-btn"),d=document.getElementById("tbl-provider-panel"),p=document.getElementById("tbl-provider-label"),u=document.getElementById("tbl-provider-clear");r==null||r.addEventListener("click",l=>{l.stopPropagation();const g=d.hidden;d.hidden=!g,r.classList.toggle("open",g)});function f(){const l=c.providers.size;p.textContent=l===0?"All providers":`${l} provider${l!==1?"s":""}`,r==null||r.classList.toggle("filtered",l>0),u&&(u.hidden=l===0)}x.querySelectorAll(".provider-cb").forEach(l=>{l.addEventListener("change",()=>{l.checked?c.providers.add(l.value):c.providers.delete(l.value),f(),c.page=0,S()})}),u==null||u.addEventListener("click",l=>{l.stopPropagation(),c.providers.clear(),x.querySelectorAll(".provider-cb").forEach(g=>g.checked=!1),f(),c.page=0,S()}),o("tbl-provider-search","input",l=>{const g=l.target.value.toLowerCase();x.querySelectorAll(".provider-option").forEach(b=>{b.hidden=g?!b.querySelector(".provider-option-name").textContent.toLowerCase().includes(g):!1})}),o("tbl-channel","change",l=>{c.channel=l.target.value,c.page=0,S()}),o("tbl-from","change",l=>{c.fromTime=l.target.value,c.page=0,S()}),o("tbl-to","change",l=>{c.toTime=l.target.value,c.page=0,S()}),o("tbl-noise","click",l=>{c.hideNoisy=!c.hideNoisy,c.page=0,l.target.classList.toggle("active",c.hideNoisy),l.target.textContent=c.hideNoisy?"Show noise":"Hide noise",S()}),o("tbl-csv","click",()=>tt(ae())),S()}function ae(){const e=c.query.toLowerCase(),t=c.fromTime?new Date(c.fromTime).getTime():null,n=c.toTime?new Date(c.toTime).getTime():null;let a=C.filter(s=>{if(c.severities.size>0&&!c.severities.has(s.severity)||c.providers.size>0&&!c.providers.has(s.provider)||c.channel&&s.channel!==c.channel||t!==null&&s.timestamp<t||n!==null&&s.timestamp>n||c.hideNoisy&&Je.has(s.provider))return!1;if(e){const i=/^\d+$/.test(e)?parseInt(e,10):null;if(i!==null){if(s.id!==i)return!1}else if(!`${s.id} ${s.provider} ${s.channel} ${s.message} ${s.severity}`.toLowerCase().includes(e))return!1}return!0});return a.sort((s,i)=>{let o=0;switch(c.sortCol){case"timestamp":o=s.timestamp-i.timestamp;break;case"severity":o=(Y[s.severity]??9)-(Y[i.severity]??9);break;case"id":o=s.id-i.id;break;case"provider":o=(s.provider||"").localeCompare(i.provider||"");break}return c.sortDir==="asc"?o:-o}),a}function S(){if(!k)return;const e=ae(),t=e.length,n=Math.max(0,Math.ceil(t/c.pageSize)-1);c.page=Math.min(c.page,n);const a=c.page*c.pageSize,s=e.slice(a,a+c.pageSize);if(!t){k.innerHTML='<div class="table-empty">No events match the current filters.</div>';return}const i=d=>`<span class="sort-arrow ${c.sortCol===d?"active":""}">${c.sortCol===d?c.sortDir==="asc"?"↑":"↓":"↕"}</span>`,o=d=>c.sortCol===d?"sort-active":"";k.innerHTML=`
  ...
  `,k.querySelectorAll("th[data-sort]").forEach(d=>{d.addEventListener("click",()=>{const p=d.dataset.sort;c.sortDir=c.sortCol===p&&c.sortDir==="asc"?"desc":"asc",c.sortCol=p,c.page=0,S()})});const r=(d,p)=>{var u;return(u=document.getElementById(d))==null?void 0:u.addEventListener("click",p)};r("pg-first",()=>{c.page=0,S()}),r("pg-prev",()=>{c.page--,S()}),r("pg-next",()=>{c.page++,S()}),r("pg-last",()=>{c.page=n,S()}),k.querySelectorAll("tbody tr[data-record]").forEach(d=>{d.addEventListener("click",p=>{if(p.target.closest(".table-event-id"))return;const u=parseInt(d.dataset.record,10);c.expandedIds.has(u)?c.expandedIds.delete(u):c.expandedIds.add(u),S()})}),k.querySelectorAll(".table-event-id").forEach(d=>{d.addEventListener("click",p=>{p.stopPropagation(),re(d.dataset.lookupId)})}),k.querySelectorAll(".ev-advanced-toggle").forEach(d=>{d.addEventListener("click",p=>{p.stopPropagation();const f=d.closest(".ev-detail-inner").querySelector(".ev-advanced-section").classList.toggle("ev-advanced-open");d.textContent=f?"Advanced ▲":"Advanced ▼"})}),k.querySelectorAll(".ev-copy-btn").forEach(d=>{d.addEventListener("click",p=>{var u;p.stopPropagation(),(u=navigator.clipboard)==null||u.writeText(d.dataset.copy).then(()=>{d.classList.add("copied"),setTimeout(()=>d.classList.remove("copied"),2e3)})})})}function et(e){const t=c.expandedIds.has(e.recordId),n=e.severity.toLowerCase(),a=Object.keys(e.data||{}),s=e.message?v(e.message.substring(0,150))+(e.message.length>150?"…":""):'<span style="color:var(--text3);font-style:italic">no message</span>',i=`
    <tr class="ev-row-${n}${t?" row-expanded":""}" data-record="${e.recordId}">
      ...
      <td class="ev-col-provider" title="${v(e.provider)}">${v(N(e.provider))}</td>
      <td class="ev-col-channel">${v(e.channel)}</td>
      <td class="ev-col-message">${s}</td>
    </tr>`;if(!t)return i;const o=e.taskName||e.task||null,r=pe(e.opcode,e.opcodeName),d=ue(e.keywords,e.keywordNames),p=de(e.userSID),u=D.get(e.id)||1,f=C.filter(w=>w.recordId!==e.recordId&&Math.abs(w.timestamp-e.timestamp)<=3e4).length,h=[["Time (local)",e.timestamp.toLocaleString()],["Time (UTC)",e.timestamp.toISOString()],["Provider",e.provider],["Channel",e.channel],["Computer",e.computer],["Record ID",e.recordId||null],["User SID",p],["Process ID",e.processId||null],["Thread ID",e.threadId||null],["Activity ID",e.activityId],["Related Act. ID",e.relatedActivityId],["Task",o],["Opcode",r],["Keywords",d]].filter(([,w])=>w),m=[["Raw Level",String(e.levelNum)],["Raw Task",e.task],["Raw Opcode",e.opcode],["Raw Keywords",e.keywords],["Version",e.version],["Qualifiers",e.qualifiers],["Provider Desc.",e.providerDescription]].filter(([,w])=>w),l=le(e.message),g=l?`<div class="ev-detail-message-wrap">
        <div class="ev-detail-message">${v(l)}</div>
       ...
       </div>`,b=e.dataAnon||[];return i+`
    <tr class="ev-detail-row">
      <td colspan="7">
        <div class="ev-detail-inner">
          ${u>1||f>0?`
          <div class="ev-occurrence-bar">
            ${u>1?`Event ${e.id} appears <strong>${u}×</strong> in this log`:""}
            ${u>1&&f>0?" &nbsp;·&nbsp; ":""}
          ...
          <div class="ev-detail-meta">
            ${h.map(([w,I])=>`
              <div class="ev-detail-field">
                <span class="ev-detail-key">${w}</span>
                <span class="ev-detail-val">${v(String(I))}</span>
              </div>`).join("")}
          </div>
          ${a.length||b.length?`
          <div class="ev-detail-data">
            ...
            ${a.map(w=>`
              <div class="ev-detail-data-row">
                <span class="ev-detail-data-key">${v(w)}</span>
                <span class="ev-detail-data-val">${v(String(e.data[w]))}</span>
              </div>`).join("")}
            ${b.map((w,I)=>`
              <div class="ev-detail-data-row">
                <span class="ev-detail-data-key ev-detail-data-key--anon">[${I}]</span>
                <span class="ev-detail-data-val">${v(String(w))}</span>
              </div>`).join("")}
          </div>`:""}
...
`),i=URL.createObjectURL(new Blob([s],{type:"text/csv;charset=utf-8;"})),o=Object.assign(document.createElement("a"),{href:i,download:`eventful-${new Date().toISOString().slice(0,10)}.csv`});document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(i)}function re(e){const t=parseInt(e,10),n=document.getElementById("lookup-panel"),a=document.getElementById("lp-body");if(!n||!a)return;const s=Ee.find(o=>o.id===t),i=C.filter(o=>o.id===t);a.innerHTML=nt(t,s,i),n.hidden=!1,a.querySelectorAll(".lp-copy-ps").forEach(o=>{o.addEventListener("click",()=>{navigator.clipboard.writeText(o.dataset.code).then(()=>{o.textContent="Copied!",setTimeout(()=>{o.textContent="Copy"},2e3)})})}),a.querySelectorAll(".lp-show-in-log").forEach(o=>{o.addEventListener("click",()=>{const r=o.dataset.filterId;R(),document.querySelectorAll(".analyzer-tab").forEach(u=>u.classList.remove("active"));const d=document.querySelector('.analyzer-tab[data-tab="events"]');d&&d.classList.add("active"),document.getElementById("incidents-section").hidden=!0,document.getElementById("events-panel").hidden=!1,c.query=r,c.page=0;const p=document.getElementById("tbl-query");p&&(p.value=r),S()})})}function R(){const e=document.getElementById("lookup-panel");e&&(e.hidden=!0)}function nt(e,t,n){var s,i,o;const a=n.length?`<button class="lp-show-in-log" data-filter-id="${e}">Show all ${n.length} occurrence${n.length!==1?"s":""} in All Events →</button>`:"";if(t){const r=((s=t.severity)==null?void 0:s.toLowerCase())??"info";return`
    ...
    <div class="lp-section">
      ...
      ${a}
    </div>`}function ce(e){const t=e.taskName||e.task||null,n=pe(e.opcode,e.opcodeName),a=ue(e.keywords,e.keywordNames),s=de(e.userSID),i=e.dataAnon||[],o=Object.keys(e.data||{}),r=D.get(e.id)||1,d=C.filter(h=>h.recordId!==e.recordId&&Math.abs(h.timestamp-e.timestamp)<=3e4).length,p=le(e.message),u=[["Time (local)",e.timestamp.toLocaleString()],["Time (UTC)",e.timestamp.toISOString()],["Provider",e.provider],["Channel",e.channel],["Computer",e.computer],["Record ID",e.recordId||null],["User SID",s],["Process ID",e.processId||null],["Thread ID",e.threadId||null],["Activity ID",e.activityId],["Task",t],["Opcode",n],["Keywords",a]].filter(([,h])=>h),f=[["Raw Level",String(e.levelNum)],["Raw Task",e.task],["Raw Opcode",e.opcode],["Raw Keywords",e.keywords],["Version",e.version]].filter(([,h])=>h);return`
        ...
        <div class="ev-detail-meta">
          ${u.map(([h,m])=>`
            <div class="ev-detail-field">
              ...
              <span class="ev-detail-val">${v(String(m))}</span>
            </div>`).join("")}
        </div>
        ${o.length||i.length?`
        <div class="ev-detail-data">
          ...
          ${o.map(h=>`
            <div class="ev-detail-data-row">
              <span class="ev-detail-data-key">${v(h)}</span>
              <span class="ev-detail-data-val">${v(String(e.data[h]))}</span>
            </div>`).join("")}
          ${i.map((h,m)=>`
            <div class="ev-detail-data-row">
              <span class="ev-detail-data-key ev-detail-data-key--anon">[${m}]</span>
              <span class="ev-detail-data-val">${v(String(h))}</span>
            </div>`).join("")}
        </div>`:""}
      </div>
      <div class="ev-detail-actions">
