...
`)}function ee(e){const t={Critical:0,Error:0,Warning:0,Info:0,Verbose:0};for(const n of e)t[n.severity]=(t[n.severity]||0)+1;return{total:e.length,...t}}function Oe(){return{total:0,Critical:0,Error:0,Warning:0,Info:0,Verbose:0}}function We(e,t){let n=100;const a=ee(e);n-=Math.min(a.Critical*15,40),n-=Math.min(a.Error*3,25),n-=Math.min(a.Warning*.5,10),n-=t.length*12;for(const s of t)s.report.confidence==="high"?n-=8:s.report.confidence==="medium"&&(n-=4);return Math.max(0,Math.min(100,Math.round(n)))}function qe(e){const t=new Set;return e.filter(n=>{const a=`${n.anchor.id}-${Math.floor(n.anchor.timestamp/1e3)}`;return t.has(a)?!1:(t.add(a),!0)})}function te(e){return e.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}ge();document.querySelectorAll(".theme-btn").forEach(e=>e.addEventListener("click",fe));const T=document.getElementById("upload-section"),ne=document.getElementById("processing-section"),se=document.getElementById("results-section"),w=document.getElementById("drop-zone"),D=document.getElementById("file-input"),K=document.getElementById("processing-text"),F=document.getElementById("overview-grid"),I=document.getElementById("incidents-section"),k=document.getElementById("event-table-wrap"),N=document.getElementById("event-log-filters-wrap"),R=document.getElementById("new-analysis-btn"),z=document.getElementById("results-sub");let C=[];D==null||D.addEventListener("change",e=>{var n;const t=(n=e.target.files)==null?void 0:n[0];t&&ie(t)});w==null||w.addEventListener("dragover",e=>{e.preventDefault(),w.classList.add("drag-over")});w==null||w.addEventListener("dragleave",()=>w.classList.remove("drag-over"));w==null||w.addEventListener("drop",e=>{var n;e.preventDefault(),w.classList.remove("drag-over");const t=(n=e.dataTransfer.files)==null?void 0:n[0];t&&ie(t)});R==null||R.addEventListener("click",Ue);var Q;(Q=document.getElementById("lp-backdrop"))==null||Q.addEventListener("click",L);var J;(J=document.getElementById("lp-close"))==null||J.addEventListener("click",L);document.addEventListener("keydown",e=>{e.key==="Escape"&&L()});async function ie(e){if(!e.name.toLowerCase().endsWith(".xml")&&e.type!=="text/xml"&&e.type!=="application/xml"){Y("Please upload an XML file exported from Windows Event Viewer.");return}M(`Reading ${e.name}…`);try{const t=await e.text();M("Parsing events…"),await P();const n=ye(t);M(`Analysing ${n.length.toLocaleString()} events…`),await P();const a=Ae(n);C=n,M("Building report…"),await P(),je(a,e.name)}catch(t){Y(t.message||"Failed to parse file."),x(T)}}function P(){return new Promise(e=>setTimeout(e,16))}function x(e){[T,ne,se].forEach(t=>{t&&(t.hidden=!0)}),e&&(e.hidden=!1)}function M(e){K&&(K.textContent=e),x(ne)}function Ue(){C=[],D&&(D.value=""),x(T)}function je(e,t){const{incidents:n,healthScore:a,computerName:s,stats:i}=e,o=document.querySelector(".results-title");if(o&&(o.textContent=t.replace(/\.xml$/i,"")),z){const l=[];s&&l.push(s),l.push(`${i.total.toLocaleString()} events`),n.length&&l.push(`${n.length} incident${n.length!==1?"s":""} detected`),z.textContent=l.join(" · ")}He(a,i),_e(n),ze(C);const r=document.getElementById("tab-inc-count"),c=document.getElementById("tab-evt-count");r&&(r.textContent=n.length),c&&(c.textContent=i.total.toLocaleString()),document.querySelectorAll(".analyzer-tab").forEach(l=>{l.addEventListener("click",()=>{document.querySelectorAll(".analyzer-tab").forEach(f=>f.classList.remove("active")),l.classList.add("active");const u=l.dataset.tab;document.getElementById("incidents-section").hidden=u!=="incidents",document.getElementById("events-panel").hidden=u!=="events"})}),x(se)}function He(e,t){if(!F)return;const n=e>=80?"#34d399":e>=60?"#f59e0b":"#f43f5e",a=e>=80?"Good":e>=60?"Degraded":"Critical";F.innerHTML=`
  ...
  `}function _e(e){if(I){if(!e.length){I.innerHTML=`
    ...
    `;return}I.innerHTML=e.map((t,n)=>Ve(t)).join(""),I.querySelectorAll(".incident-toggle").forEach(t=>{t.addEventListener("click",n=>{if(n.target.closest("[data-lookup-id]"))return;const s=t.closest(".incident-card").querySelector(".incident-body"),i=t.querySelector(".incident-chevron"),o=!s.hidden;s.hidden=o,i.classList.toggle("open",!o)})}),I.querySelectorAll(".copy-summary-btn").forEach(t=>{t.addEventListener("click",()=>{const n=t.dataset.summary;navigator.clipboard.writeText(n).then(()=>{t.textContent="Copied!",t.classList.add("copied"),setTimeout(()=>{t.textContent="Copy for ticket",t.classList.remove("copied")},2e3)})})}),I.querySelectorAll("[data-lookup-id]").forEach(t=>{t.addEventListener("click",()=>ae(t.dataset.lookupId))})}}function Ve(e,t){var f,h;const{anchor:n,windowEvents:a,topContributors:s,signatureResult:i,report:o}=e,r=(f=i[0])==null?void 0:f.signature,c=o.confidence,l=Ze(n.severity),u=c==="high"?"conf-high":c==="medium"?"conf-medium":"conf-low";return`
    <div class="incident-card">
      <div class="incident-header ${l} incident-toggle">
        ...
        <div class="incident-header-right">
          <span class="conf-badge ${u}">${c}</span>
  ...
  `}function Ge(e,t){const n=[...e,t].sort((i,o)=>i.timestamp-o.timestamp),s=n.length>12?[...n.slice(0,6),{_ellipsis:!0,count:n.length-10},...n.slice(-4)]:n;return`
    <div class="incident-section">
      ...
      <div class="mini-timeline">
        ${s.map(i=>{var r;if(i._ellipsis)return`<div class="timeline-ellipsis">· · · ${i.count} more events · · ·</div>`;const o=i===t;return`
            <div class="timeline-item ${o?"timeline-anchor":""}" data-lookup-id="${i.id}" title="Look up Event ${i.id}">
              <div class="tl-dot sev-${(r=i.severity)==null?void 0:r.toLowerCase()}"></div>
              <div class="tl-content">
                <span class="tl-time">${re(i.timestamp)}</span>
                <span class="tl-id">${i.id}</span>
  ...
  `;const o=(l,u,f)=>{var h;return(h=document.getElementById(l))==null?void 0:h.addEventListener(u,f)};o("tbl-query","input",l=>{d.query=l.target.value,d.page=0,y()}),N.querySelectorAll(".tbl-sev-cb").forEach(l=>{l.addEventListener("change",()=>{l.checked?d.severities.add(l.value):d.severities.delete(l.value),l.closest(".sev-chip").classList.toggle("active",l.checked),d.page=0,y()})}),o("tbl-provider","change",l=>{d.provider=l.target.value,d.page=0,y()}),o("tbl-channel","change",l=>{d.channel=l.target.value,d.page=0,y()}),o("tbl-from","change",l=>{d.fromTime=l.target.value,d.page=0,y()}),o("tbl-to","change",l=>{d.toTime=l.target.value,d.page=0,y()}),o("tbl-noise","click",l=>{d.hideNoisy=!d.hideNoisy,d.page=0,l.target.classList.toggle("active",d.hideNoisy),l.target.textContent=d.hideNoisy?"Show noise":"Hide noise",y()}),o("tbl-csv","click",()=>Ye(oe())),y()}function oe(){const e=d.query.toLowerCase(),t=d.fromTime?new Date(d.fromTime).getTime():null,n=d.toTime?new Date(d.toTime).getTime():null;let a=C.filter(s=>{if(d.severities.size>0&&!d.severities.has(s.severity)||d.provider&&s.provider!==d.provider||d.channel&&s.channel!==d.channel||t!==null&&s.timestamp<t||n!==null&&s.timestamp>n||d.hideNoisy&&Fe.has(s.provider))return!1;if(e){const i=/^\d+$/.test(e)?parseInt(e,10):null;if(i!==null){if(s.id!==i)return!1}else if(!`${s.id} ${s.provider} ${s.channel} ${s.message} ${s.severity}`.toLowerCase().includes(e))return!1}return!0});return a.sort((s,i)=>{let o=0;switch(d.sortCol){case"timestamp":o=s.timestamp-i.timestamp;break;case"severity":o=(X[s.severity]??9)-(X[i.severity]??9);break;case"id":o=s.id-i.id;break;case"provider":o=(s.provider||"").localeCompare(i.provider||"");break}return d.sortDir==="asc"?o:-o}),a}function y(){if(!k)return;const e=oe(),t=e.length,n=Math.max(0,Math.ceil(t/d.pageSize)-1);d.page=Math.min(d.page,n);const a=d.page*d.pageSize,s=e.slice(a,a+d.pageSize);if(!t){k.innerHTML='<div class="table-empty">No events match the current filters.</div>';return}const i=c=>`<span class="sort-arrow ${d.sortCol===c?"active":""}">${d.sortCol===c?d.sortDir==="asc"?"↑":"↓":"↕"}</span>`,o=c=>d.sortCol===c?"sort-active":"";k.innerHTML=`
  ...
  `,k.querySelectorAll("th[data-sort]").forEach(c=>{c.addEventListener("click",()=>{const l=c.dataset.sort;d.sortDir=d.sortCol===l&&d.sortDir==="asc"?"desc":"asc",d.sortCol=l,d.page=0,y()})});const r=(c,l)=>{var u;return(u=document.getElementById(c))==null?void 0:u.addEventListener("click",l)};r("pg-first",()=>{d.page=0,y()}),r("pg-prev",()=>{d.page--,y()}),r("pg-next",()=>{d.page++,y()}),r("pg-last",()=>{d.page=n,y()}),k.querySelectorAll("tbody tr[data-record]").forEach(c=>{c.addEventListener("click",l=>{if(l.target.closest(".table-event-id"))return;const u=parseInt(c.dataset.record,10);d.expandedIds.has(u)?d.expandedIds.delete(u):d.expandedIds.add(u),y()})}),k.querySelectorAll(".table-event-id").forEach(c=>{c.addEventListener("click",l=>{l.stopPropagation(),ae(c.dataset.lookupId)})}),k.querySelectorAll(".ev-advanced-toggle").forEach(c=>{c.addEventListener("click",l=>{l.stopPropagation();const f=c.closest(".ev-detail-inner").querySelector(".ev-advanced-section").classList.toggle("ev-advanced-open");c.textContent=f?"Advanced ▲":"Advanced ▼"})})}function Xe(e){var v;const t=d.expandedIds.has(e.recordId),n=e.severity.toLowerCase(),a=Object.keys(e.data||{}),s=e.message?p(e.message.substring(0,150))+(e.message.length>150?"…":""):'<span style="color:var(--text3);font-style:italic">no message</span>',i=`
    ...
    </tr>`;if(!t)return i;const o=e.taskName||e.task||null,r=e.opcodeName||e.opcode||null,c=(v=e.keywordNames)!=null&&v.length?e.keywordNames.join(", "):e.keywords||null,l=[["Time (local)",e.timestamp.toLocaleString()],["Time (UTC)",e.timestamp.toISOString()],["Provider",e.provider],["Channel",e.channel],["Computer",e.computer],["Record ID",e.recordId||null],["User SID",e.userSID],["Process ID",e.processId||null],["Thread ID",e.threadId||null],["Activity ID",e.activityId],["Related Act. ID",e.relatedActivityId],["Task",o],["Opcode",r],["Keywords",c]].filter(([,g])=>g),u=[["Raw Level",String(e.levelNum)],["Raw Task",e.task],["Raw Opcode",e.opcode],["Raw Keywords",e.keywords],["Version",e.version],["Qualifiers",e.qualifiers],["Provider Desc.",e.providerDescription]].filter(([,g])=>g),f=e.message?`<div class="ev-detail-message">${p(e.message)}</div>`:`<div class="ev-detail-message ev-no-message">
...
`),i=URL.createObjectURL(new Blob([s],{type:"text/csv;charset=utf-8;"})),o=Object.assign(document.createElement("a"),{href:i,download:`eventful-${new Date().toISOString().slice(0,10)}.csv`});document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(i)}function ae(e){const t=parseInt(e,10),n=document.getElementById("lookup-panel"),a=document.getElementById("lp-body");if(!n||!a)return;const s=we.find(o=>o.id===t),i=C.filter(o=>o.id===t);a.innerHTML=Qe(t,s,i),n.hidden=!1,a.querySelectorAll(".lp-copy-ps").forEach(o=>{o.addEventListener("click",()=>{navigator.clipboard.writeText(o.dataset.code).then(()=>{o.textContent="Copied!",setTimeout(()=>{o.textContent="Copy"},2e3)})})}),a.querySelectorAll(".lp-advanced-toggle").forEach(o=>{o.addEventListener("click",()=>{const c=o.nextElementSibling.classList.toggle("lp-advanced-open");o.textContent=c?"Advanced ▲":"Advanced ▼"})}),a.querySelectorAll(".lp-show-in-log").forEach(o=>{o.addEventListener("click",()=>{const r=o.dataset.filterId;L(),document.querySelectorAll(".analyzer-tab").forEach(u=>u.classList.remove("active"));const c=document.querySelector('.analyzer-tab[data-tab="events"]');c&&c.classList.add("active"),document.getElementById("incidents-section").hidden=!0,document.getElementById("events-panel").hidden=!1,d.query=r,d.page=0;const l=document.getElementById("tbl-query");l&&(l.value=r),y()})})}function L(){const e=document.getElementById("lookup-panel");e&&(e.hidden=!0)}function Qe(e,t,n){var i,o,r;let a="";if(t){const c=((i=t.severity)==null?void 0:i.toLowerCase())??"info";a+=`
    ...
    <div class="lp-section">
      ...
      ${s.map((c,l)=>{var m;const u=c.taskName||c.opcode||null,f=c.opcodeName||c.opcode||null,h=(m=c.keywordNames)!=null&&m.length?c.keywordNames.join(", "):c.keywords||null,v=c.dataAnon||[],g=[["Raw Level",String(c.levelNum)],["Raw Task",c.task],["Raw Opcode",c.opcode],["Raw Keywords",c.keywords],["Version",c.version],["Qualifiers",c.qualifiers],["Provider Desc.",c.providerDescription],["Related Act. ID",c.relatedActivityId]].filter(([,$])=>$);return`
        ${l>0?'<div class="lp-raw-divider"></div>':""}
        <div class="lp-raw-fields">
          ${S("Time",c.timestamp.toLocaleString())}
          ${S("Severity",`<span class="sev-badge sev-badge-${c.severity.toLowerCase()}">${c.severity}</span>`)}
          ${S("Provider",p(A(c.provider)))}
          ${S("Channel",p(c.channel))}
          ${S("Computer",p(c.computer||"—"))}
          ${S("Record ID",String(c.recordId||"—"))}
          ${c.processId?S("Process ID",String(c.processId)):""}
          ${c.threadId?S("Thread ID",String(c.threadId)):""}
          ${c.userSID?S("User SID",p(c.userSID)):""}
          ${c.activityId?S("Activity ID",p(c.activityId)):""}
          ${u?S("Task",p(u)):""}
          ${f?S("Opcode",p(f)):""}
          ${h?S("Keywords",p(h)):""}
        </div>
