...
`)}function ie(e){const t={Critical:0,Error:0,Warning:0,Info:0,Verbose:0};for(const n of e)t[n.severity]=(t[n.severity]||0)+1;return{total:e.length,...t}}function _e(){return{total:0,Critical:0,Error:0,Warning:0,Info:0,Verbose:0}}function Ve(e,t){let n=100;const a=ie(e);n-=Math.min(a.Critical*15,40),n-=Math.min(a.Error*3,25),n-=Math.min(a.Warning*.5,10),n-=t.length*12;for(const s of t)s.report.confidence==="high"?n-=8:s.report.confidence==="medium"&&(n-=4);return Math.max(0,Math.min(100,Math.round(n)))}function Ke(e){const t=new Set;return e.filter(n=>{const a=`${n.anchor.id}-${Math.floor(n.anchor.timestamp/1e3)}`;return t.has(a)?!1:(t.add(a),!0)})}function oe(e){return e.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}$e();document.querySelectorAll(".theme-btn").forEach(e=>e.addEventListener("click",ke));const P=document.getElementById("upload-section"),ae=document.getElementById("processing-section"),re=document.getElementById("results-section"),y=document.getElementById("drop-zone"),L=document.getElementById("file-input"),Y=document.getElementById("processing-text"),Q=document.getElementById("overview-grid"),E=document.getElementById("incidents-section"),x=document.getElementById("event-table-wrap"),D=document.getElementById("event-log-filters-wrap"),O=document.getElementById("new-analysis-btn"),J=document.getElementById("results-sub");let C=[],A=new Map;L==null||L.addEventListener("change",e=>{var n;const t=(n=e.target.files)==null?void 0:n[0];t&&ce(t)});y==null||y.addEventListener("dragover",e=>{e.preventDefault(),y.classList.add("drag-over")});y==null||y.addEventListener("dragleave",()=>y.classList.remove("drag-over"));y==null||y.addEventListener("drop",e=>{var n;e.preventDefault(),y.classList.remove("drag-over");const t=(n=e.dataTransfer.files)==null?void 0:n[0];t&&ce(t)});O==null||O.addEventListener("click",Ge);var te;(te=document.getElementById("lp-backdrop"))==null||te.addEventListener("click",q);var ne;(ne=document.getElementById("lp-close"))==null||ne.addEventListener("click",q);document.addEventListener("keydown",e=>{e.key==="Escape"&&q()});async function ce(e){if(!e.name.toLowerCase().endsWith(".xml")&&e.type!=="text/xml"&&e.type!=="application/xml"){ee("Please upload an XML file exported from Windows Event Viewer.");return}N(`Reading ${e.name}…`);try{const t=await e.text();N("Parsing events…"),await W();const n=Ce(t);N(`Analysing ${n.length.toLocaleString()} events…`),await W();const a=Pe(n);C=n,A=new Map;for(const s of n)A.set(s.id,(A.get(s.id)||0)+1);N("Building report…"),await W(),ze(a,e.name)}catch(t){ee(t.message||"Failed to parse file."),R(P)}}function W(){return new Promise(e=>setTimeout(e,16))}function R(e){[P,ae,re].forEach(t=>{t&&(t.hidden=!0)}),e&&(e.hidden=!1)}function N(e){Y&&(Y.textContent=e),R(ae)}function Ge(){C=[],A=new Map,L&&(L.value=""),R(P)}function ze(e,t){const{incidents:n,healthScore:a,computerName:s,stats:i}=e,o=document.querySelector(".results-title");if(o&&(o.textContent=t.replace(/\.xml$/i,"")),J){const p=[];s&&p.push(s),p.push(`${i.total.toLocaleString()} events`),n.length&&p.push(`${n.length} incident${n.length!==1?"s":""} detected`),J.textContent=p.join(" · ")}Fe(a,i),Xe(n),Ze(C);const r=document.getElementById("tab-inc-count"),l=document.getElementById("tab-evt-count");r&&(r.textContent=n.length),l&&(l.textContent=i.total.toLocaleString()),document.querySelectorAll(".analyzer-tab").forEach(p=>{p.addEventListener("click",()=>{document.querySelectorAll(".analyzer-tab").forEach(g=>g.classList.remove("active")),p.classList.add("active");const h=p.dataset.tab;document.getElementById("incidents-section").hidden=h!=="incidents",document.getElementById("events-panel").hidden=h!=="events"})}),R(re)}function Fe(e,t){if(!Q)return;const n=e>=80?"#34d399":e>=60?"#f59e0b":"#f43f5e",a=e>=80?"Good":e>=60?"Degraded":"Critical";Q.innerHTML=`
  ...
  `}function Xe(e){if(E){if(!e.length){E.innerHTML=`
    ...
    <div class="incident-card">
      <div class="incident-header ${p} incident-toggle">
        <div class="incident-header-left">
          ...
          <div>
            <div class="incident-title">${(r==null?void 0:r.name)??lt(n)}</div>
            ...
            ${s.slice(0,6).map(({event:v,score:k})=>`
              <div class="evidence-wrap">
                <div class="evidence-item">
                  ...
                  <span class="ev-provider">${u(B(v.provider))}</span>
                  <span class="ev-time">${ge(v.timestamp)}</span>
  ...
  `}function Qe(e,t){const n=[...e,t].sort((i,o)=>i.timestamp-o.timestamp),s=n.length>12?[...n.slice(0,6),{_ellipsis:!0,count:n.length-10},...n.slice(-4)]:n;return`
    <div class="incident-section">
      <div class="incident-section-label">Timeline (${e.length} events in ${Je}-min window)</div>
            ...
            <div class="timeline-item-wrap">
              <div class="timeline-item ${o?"timeline-anchor":""}">
                ...
                <div class="tl-content">
                  <span class="tl-time">${ge(i.timestamp)}</span>
                  <span class="tl-id" data-lookup-id="${i.id}" title="Look up Event ${i.id}">${i.id}</span>
                  <span class="tl-provider">${u(B(i.provider))}</span>
  ...
  `}const Je=15,le=new Set(["Microsoft-Windows-TaskScheduler","Microsoft-Windows-WindowsUpdateClient","Microsoft-Windows-Bits-Client","Microsoft-Windows-GroupPolicy","Microsoft-Windows-UserPnp","Microsoft-Windows-WER-SystemErrorReporting","Microsoft-Windows-Diagnostics-Performance","Microsoft-Windows-DistributedCOM","Microsoft-Windows-Security-SPP","Microsoft-Windows-Defrag","Microsoft-Windows-Power-Troubleshooter"]),Z={Critical:0,Error:1,Warning:2,Info:3,Verbose:4},c={sortCol:"timestamp",sortDir:"asc",page:0,pageSize:100,query:"",severities:new Set,providers:new Set,channel:"",fromTime:"",toTime:"",hideNoisy:!1,expandedIds:new Set};document.addEventListener("click",e=>{const t=document.getElementById("tbl-provider-panel"),n=document.getElementById("tbl-provider-btn");t&&!t.hidden&&!t.contains(e.target)&&!(n!=null&&n.contains(e.target))&&(t.hidden=!0,n==null||n.classList.remove("open"))});function Ze(e){var k,w;if(!D||!x)return;Object.assign(c,{sortCol:"timestamp",sortDir:"asc",page:0,query:"",severities:new Set,providers:new Set,channel:"",fromTime:"",toTime:"",hideNoisy:!1,expandedIds:new Set});const t=[...new Set(e.map(d=>d.provider).filter(Boolean))].sort(),n=[...new Set(e.map(d=>d.channel).filter(Boolean))].sort(),a=d=>d?new Date(d-d.getTimezoneOffset()*6e4).toISOString().slice(0,16):"",s=(k=e[0])==null?void 0:k.timestamp,i=(w=e[e.length-1])==null?void 0:w.timestamp;D.innerHTML=`
    <div class="event-log-filters">
          ...
          <div class="provider-option-list">
            ${t.map(d=>`
              <label class="provider-option">
                <input type="checkbox" class="provider-cb" value="${u(d)}" />
                <span class="provider-option-name" title="${u(d)}">${u(B(d))}</span>
              </label>`).join("")}
          </div>
        </div>
      </div>
      ...
      <select id="tbl-channel" class="filter-control filter-control-select">
        ...
        ${n.map(d=>`<option value="${u(d)}">${u(d)}</option>`).join("")}
      </select>
  ...
  `;const o=(d,f,$)=>{var b;return(b=document.getElementById(d))==null?void 0:b.addEventListener(f,$)};o("tbl-query","input",d=>{c.query=d.target.value,c.page=0,S()}),D.querySelectorAll(".tbl-sev-cb").forEach(d=>{d.addEventListener("change",()=>{d.checked?c.severities.add(d.value):c.severities.delete(d.value),d.closest(".sev-chip").classList.toggle("active",d.checked),c.page=0,v(),S()})});const r=document.getElementById("tbl-provider-btn"),l=document.getElementById("tbl-provider-panel"),p=document.getElementById("tbl-provider-label"),h=document.getElementById("tbl-provider-clear"),g=document.getElementById("tbl-provider-select-all");r==null||r.addEventListener("click",d=>{d.stopPropagation();const f=l.hidden;l.hidden=!f,r.classList.toggle("open",f)});function m(){const d=c.providers.size;p.textContent=d===0?"All providers":`${d} provider${d!==1?"s":""}`,r==null||r.classList.toggle("filtered",d>0)}function v(){const d=c.fromTime?new Date(c.fromTime).getTime():null,f=c.toTime?new Date(c.toTime).getTime():null,$=new Set(C.filter(b=>!(c.severities.size>0&&!c.severities.has(b.severity)||c.channel&&b.channel!==c.channel||d!==null&&b.timestamp<d||f!==null&&b.timestamp>f||c.hideNoisy&&le.has(b.provider))).map(b=>b.provider).filter(Boolean));D.querySelectorAll(".provider-option").forEach(b=>{const T=b.querySelector(".provider-cb"),M=$.has(T.value);b.classList.toggle("provider-option-unavailable",!M),T.disabled=!M,!M&&T.checked&&(T.checked=!1,c.providers.delete(T.value))}),m()}D.querySelectorAll(".provider-cb").forEach(d=>{d.addEventListener("change",()=>{d.checked?c.providers.add(d.value):c.providers.delete(d.value),m(),c.page=0,S()})}),h==null||h.addEventListener("click",d=>{d.stopPropagation(),c.providers.clear(),D.querySelectorAll(".provider-cb").forEach(f=>f.checked=!1),m(),c.page=0,S()}),g==null||g.addEventListener("click",d=>{d.stopPropagation(),D.querySelectorAll(".provider-option:not([hidden]) .provider-cb").forEach(f=>{f.checked=!0,c.providers.add(f.value)}),m(),c.page=0,S()}),o("tbl-provider-search","input",d=>{const f=d.target.value.toLowerCase();D.querySelectorAll(".provider-option").forEach($=>{$.hidden=f?!$.querySelector(".provider-option-name").textContent.toLowerCase().includes(f):!1})}),o("tbl-channel","change",d=>{c.channel=d.target.value,c.page=0,v(),S()}),o("tbl-from","change",d=>{c.fromTime=d.target.value,c.page=0,v(),S()}),o("tbl-to","change",d=>{c.toTime=d.target.value,c.page=0,v(),S()}),o("tbl-noise","click",d=>{c.hideNoisy=!c.hideNoisy,c.page=0,d.target.classList.toggle("active",c.hideNoisy),d.target.textContent=c.hideNoisy?"Show noise":"Hide noise",v(),S()}),o("tbl-csv","click",()=>tt(de())),v(),S()}function de(){const e=c.query.toLowerCase(),t=c.fromTime?new Date(c.fromTime).getTime():null,n=c.toTime?new Date(c.toTime).getTime():null;let a=C.filter(s=>{if(c.severities.size>0&&!c.severities.has(s.severity)||c.providers.size>0&&!c.providers.has(s.provider)||c.channel&&s.channel!==c.channel||t!==null&&s.timestamp<t||n!==null&&s.timestamp>n||c.hideNoisy&&le.has(s.provider))return!1;if(e){const i=/^\d+$/.test(e)?parseInt(e,10):null;if(i!==null){if(s.id!==i)return!1}else if(!`${s.id} ${s.provider} ${s.channel} ${s.message} ${s.severity}`.toLowerCase().includes(e))return!1}return!0});return a.sort((s,i)=>{let o=0;switch(c.sortCol){case"timestamp":o=s.timestamp-i.timestamp;break;case"severity":o=(Z[s.severity]??9)-(Z[i.severity]??9);break;case"id":o=s.id-i.id;break;case"provider":o=(s.provider||"").localeCompare(i.provider||"");break}return c.sortDir==="asc"?o:-o}),a}function S(){if(!x)return;const e=de(),t=e.length,n=Math.max(0,Math.ceil(t/c.pageSize)-1);c.page=Math.min(c.page,n);const a=c.page*c.pageSize,s=e.slice(a,a+c.pageSize);if(!t){x.innerHTML='<div class="table-empty">No events match the current filters.</div>';return}const i=l=>`<span class="sort-arrow ${c.sortCol===l?"active":""}">${c.sortCol===l?c.sortDir==="asc"?"↑":"↓":"↕"}</span>`,o=l=>c.sortCol===l?"sort-active":"";x.innerHTML=`
  ...
  `,x.querySelectorAll("th[data-sort]").forEach(l=>{l.addEventListener("click",()=>{const p=l.dataset.sort;c.sortDir=c.sortCol===p&&c.sortDir==="asc"?"desc":"asc",c.sortCol=p,c.page=0,S()})});const r=(l,p)=>{var h;return(h=document.getElementById(l))==null?void 0:h.addEventListener("click",p)};r("pg-first",()=>{c.page=0,S()}),r("pg-prev",()=>{c.page--,S()}),r("pg-next",()=>{c.page++,S()}),r("pg-last",()=>{c.page=n,S()}),x.querySelectorAll("tbody tr[data-record]").forEach(l=>{l.addEventListener("click",p=>{if(p.target.closest(".table-event-id"))return;const h=parseInt(l.dataset.record,10);c.expandedIds.has(h)?c.expandedIds.delete(h):c.expandedIds.add(h),S()})}),x.querySelectorAll(".table-event-id").forEach(l=>{l.addEventListener("click",p=>{p.stopPropagation(),pe(l.dataset.lookupId)})}),x.querySelectorAll(".ev-advanced-toggle").forEach(l=>{l.addEventListener("click",p=>{p.stopPropagation();const g=l.closest(".ev-detail-inner").querySelector(".ev-advanced-section").classList.toggle("ev-advanced-open");l.textContent=g?"Advanced ▲":"Advanced ▼"})}),x.querySelectorAll(".ev-copy-btn").forEach(l=>{l.addEventListener("click",p=>{var h;p.stopPropagation(),(h=navigator.clipboard)==null||h.writeText(l.dataset.copy).then(()=>{l.classList.add("copied"),setTimeout(()=>l.classList.remove("copied"),2e3)})})})}function et(e){const t=c.expandedIds.has(e.recordId),n=e.severity.toLowerCase(),a=Object.keys(e.data||{}),s=e.message?u(e.message.substring(0,150))+(e.message.length>150?"…":""):'<span style="color:var(--text3);font-style:italic">no message</span>',i=`
    <tr class="ev-row-${n}${t?" row-expanded":""}" data-record="${e.recordId}">
      ...
      <td class="ev-col-provider" title="${u(e.provider)}">${u(B(e.provider))}</td>
      <td class="ev-col-channel">${u(e.channel)}</td>
      <td class="ev-col-message">${s}</td>
    </tr>`;if(!t)return i;const o=e.taskName||e.task||null,r=me(e.opcode,e.opcodeName),l=fe(e.keywords,e.keywordNames),p=ve(e.userSID),h=A.get(e.id)||1,g=C.filter(f=>f.recordId!==e.recordId&&Math.abs(f.timestamp-e.timestamp)<=3e4).length,m=[["Time (local)",e.timestamp.toLocaleString()],["Time (UTC)",e.timestamp.toISOString()],["Provider",e.provider],["Channel",e.channel],["Computer",e.computer],["Record ID",e.recordId||null],["User SID",p],["Process ID",e.processId||null],["Thread ID",e.threadId||null],["Activity ID",e.activityId],["Related Act. ID",e.relatedActivityId],["Task",o],["Opcode",r],["Keywords",l]].filter(([,f])=>f),v=[["Raw Level",String(e.levelNum)],["Raw Task",e.task],["Raw Opcode",e.opcode],["Raw Keywords",e.keywords],["Version",e.version],["Qualifiers",e.qualifiers],["Provider Desc.",e.providerDescription]].filter(([,f])=>f),k=he(e.message),w=k?`<div class="ev-detail-message-wrap">
...
`),i=URL.createObjectURL(new Blob([s],{type:"text/csv;charset=utf-8;"})),o=Object.assign(document.createElement("a"),{href:i,download:`eventful-${new Date().toISOString().slice(0,10)}.csv`});document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(i)}function pe(e){const t=parseInt(e,10),n=document.getElementById("lookup-panel"),a=document.getElementById("lp-body");if(!n||!a)return;const s=Ee.find(o=>o.id===t),i=C.filter(o=>o.id===t);a.innerHTML=nt(t,s,i),n.hidden=!1,a.querySelectorAll(".lp-copy-ps").forEach(o=>{o.addEventListener("click",()=>{navigator.clipboard.writeText(o.dataset.code).then(()=>{o.textContent="Copied!",setTimeout(()=>{o.textContent="Copy"},2e3)})})}),a.querySelectorAll(".lp-show-in-log").forEach(o=>{o.addEventListener("click",()=>{const r=o.dataset.filterId;q(),document.querySelectorAll(".analyzer-tab").forEach(h=>h.classList.remove("active"));const l=document.querySelector('.analyzer-tab[data-tab="events"]');l&&l.classList.add("active"),document.getElementById("incidents-section").hidden=!0,document.getElementById("events-panel").hidden=!1,c.query=r,c.page=0;const p=document.getElementById("tbl-query");p&&(p.value=r),S()})})}function q(){const e=document.getElementById("lookup-panel");e&&(e.hidden=!0)}function nt(e,t,n){var s,i,o;const a=n.length?`<button class="lp-show-in-log" data-filter-id="${e}">Show all ${n.length} occurrence${n.length!==1?"s":""} in All Events →</button>`:"";if(t){const r=((s=t.severity)==null?void 0:s.toLowerCase())??"info";return`
    ...
    </div>`}const st={"S-1-1-0":"Everyone","S-1-5-7":"Anonymous","S-1-5-18":"SYSTEM","S-1-5-19":"LOCAL SERVICE","S-1-5-20":"NETWORK SERVICE","S-1-5-32-544":"Administrators","S-1-5-32-545":"Users","S-1-5-32-546":"Guests"};function ve(e){if(!e)return null;const t=st[e];return t?`${t} (${e})`:e}const it={2:"The system cannot find the file specified",3:"The system cannot find the path specified",5:"Access is denied",32:"The process cannot access the file because it is being used by another process",1053:"The service did not respond to the start or control request in a timely fashion",1055:"The service database is locked",1056:"An instance of the service is already running",1058:"The service cannot be started — it is disabled or has no enabled devices associated with it",1060:"The specified service does not exist as an installed service",1061:"The service cannot accept control messages at this time",1067:"The process terminated unexpectedly",1068:"The dependency service or group failed to start",1069:"The service did not start due to a logon failure",1072:"The specified service has been marked for deletion",1073:"The specified service already exists",1326:"Logon failure: unknown user name or bad password"};function he(e){return e&&e.replace(/%%(\d+)/g,(t,n)=>{const a=it[+n];return a?`${a} (%%${n})`:t})}const ot={0:"Info",1:"Start",2:"Stop",3:"DC Start",4:"DC Stop",5:"Extension",6:"Reply",7:"Resume",8:"Suspend",9:"Send",240:"Disconnect",241:"Connect"};function me(e,t){return t||(e==null||e===""?null:ot[String(e)]||String(e))}const at={"0x8000000000000000":"Audit Failure","0x4000000000000000":"Audit Success","0x8080000000000000":"Classic, Audit Failure","0x4080000000000000":"Classic, Audit Success","0x0080000000000000":"Classic","0x0000000000000000":"None"};function fe(e,t){return t!=null&&t.length?t.join(", "):e?at[e.toLowerCase()]??e:null}function ee(e){const t=y==null?void 0:y.querySelector(".upload-error");t&&t.remove();const n=document.createElement("div");n.className="upload-error",n.textContent=e,y==null||y.appendChild(n),R(P)}function u(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"):""}function B(e){return e?e.replace(/^Microsoft-Windows-/i,"").replace(/^Microsoft-/i,""):"—"}function ge(e){return e.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}function rt(e){return e.toLocaleString([],{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"})}function ct(e){return`sev-header-${(e==null?void 0:e.toLowerCase())??"info"}`}function lt(e){return{41:"Unexpected System Reboot",6008:"Unexpected Shutdown Detected",1001:"System Crash (BSOD)",1e3:"Application Crash",7024:"Critical Service Failure"}[e.id]??`Event ${e.id}`}
